# -*- coding: utf-8 -*-
"""Copy of tf.colorimageworks.001.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1MD0pAjIsPuBYHs0x_gP4esiJf1hskS0t
"""
...
from tensorflow.keras.layers import Conv2D, Dense, Flatten, Reshape, LeakyReLU, Dropout, UpSampling2D, Input, GlobalAveragePooling2D
...
dataset, dataset_info = tfds.load('oxford_flowers102', with_info=True, as_supervised=False)
print(dataset_info)
ds = dataset['train']
...
def build_generator():
  ...
  #model.build(input_shape=[res_pix, res_pix, 3])
  model.add(Input(shape=(res_pix, res_pix, 3)))
  #model.add(Dense(3))
  #Flatten(input_shape=(res_pix, res_pix, 3))
  #model.add(Reshape((res_pix, res_pix , 3)))
  #model.add(Conv2D(32, kernel_size=5, input_shape = (res_pix, res_pix, 3), input_dim=3))
  #model.add(tf.keras.layers.Dense(32, activation='relu'))
  #model.add(tf.keras.Input(shape=(res_pix, res_pix), input_dim=3))
  #model.add(tf.keras.layers.Dense(res_pix, res_pix, 3, activation='relu'))


  #model.add(Dense(3,res_pix, res_pix))
...
img = generator.predict(np.random.randn(num_images, res_pix, res_pix, 3))
...
def build_discriminator():
  ...
  #model.add(Conv2D(32, 5, input_shape = (res_pix, res_pix, 3), input_dim=3))
  model.add(Input(shape=(res_pix, res_pix, 3)))
  #model.add(Reshape((res_pix, res_pix, 3)))
  model.add(LeakyReLU(0.2))
...
class ColorImageWorks(Model):
  ...
  def train_step(self, batch):
    real_images = batch
    fake_images = self.generator(tf.random.normal((batch_size, res_pix, res_pix, 3)), training=False)
    ...
    with tf.GradientTape() as g_tape:
      ...
      gen_images = self.generator(tf.random.normal((batch_size, res_pix, res_pix, 3)), training=True)
...
colorimage = ColorImageWorks(generator, discriminator)

colorimage.compile(g_opt, d_opt, g_loss, d_loss)

class ModelMonitor(Callback):
  def __init__(self, num_images=num_images, res_pix=res_pix):
    self.num_images = num_images
    self.res_pix = res_pix
    self.latent_dim = res_pix

  def on_epoch_end(self, epoch, logs=None):
    print('-- epoch end --')
    random_latent_vectors = tf.random.uniform((self.num_images, self.res_pix, self.res_pix, 3))
    ...
    for i in range(self.num_images):
      ...
      plt.figure(figsize=(self.res_pix, self.res_pix))
      plt.imshow(file_img)
      plt.axis('off')
      plt.show()
...
if model_monitor == True:
  hist = colorimage.fit(ds, epochs=epochs, callbacks=[ModelMonitor()])
else:
  hist = colorimage.fit(ds, epochs=epochs)
...
def generate_images(num_x, num_y, size_x, size_y):
  generated_images = generator.predict(tf.random.normal((batch_size, res_pix, res_pix, 3)))
